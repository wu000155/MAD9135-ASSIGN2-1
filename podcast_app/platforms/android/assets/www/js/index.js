...
var app = {
    ...
    onDeviceReady: function () {
        ...
        if (app.detectTouchSupport()) {
            ...
            $('.back,.addnewBtn,.setDone').bind('touchend', app.handleTouchEnd);
    ...
    downloadNewPodcasts: function (podcastXml) {
        ...
        if (isRecord) {
            ...
            for (var i = 0; i < 2; i++) {
                ...
                var titleWhole = $(podcastXml).find('item>title').eq(i).text();
                var fileTransfer = new FileTransfer();
                var uri = encodeURI($(podcastXml).find('item>enclosure').eq(i).attr('url'));
                var specialChars = "#:\./|-#:\./|-#:\./|-#:\./|-#:\./|-#:\./|-#:\./|-#:\./|-#:\./|-#:\./|-"
                ...
                var date = $(podcastXml).find("item>pubDate").eq(i).text().split('+');
                var podcastInfo = '{'
                podcastInfo += '"channel":"' + $(podcastXml).find('channel>title').text() + '",'
                podcastInfo += '"title":"' + titleWhole + '",'
                podcastInfo += '"img":"' + $(podcastXml).find('item>image').eq(i).attr('href') + '",'
                podcastInfo += '"duration":"' + $(podcastXml).find("item>duration").eq(i).text() + '",'
...
//    },
    ...
    networkfail: function () {
        ...
        checkOnlineInterval= setInterval(app.checkOnline, 10000);
    },
